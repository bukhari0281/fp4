...
type UserService interface {
	...
	FindUserByID(userID string) (*_user.UserLoginResponse, error)
	FindUserByIDPolos(userID string) (*_user.UserResponse, error)
}
...
func (c *userService) FindUserByIDPolos(userID string) (*_user.UserResponse, error) {
...
func (c *userService) FindUserByID(userID string) (*_user.UserLoginResponse, error) {
	user, err := c.userRepo.FindByUserID(userID)
	...
	userResponse := _user.UserLoginResponse{}
	err = smapping.FillStruct(&userResponse, smapping.MapFields(&user))
	if err != nil {
		return nil, err
	}
